...
const fromSignaturePage = marketingPageByKey('fromSignature');

export const metadata = createPageMetadata({
  title: fromSignaturePage.title,
  description: fromSignaturePage.description,
  path: fromSignaturePage.path,
});
...
export default function FromSignatureLandingPage() {
    ...
    <div className="relative isolate">
      <JsonLd
        data={[
          webPageJsonLd({
            path: fromSignaturePage.path,
            name: fromSignaturePage.title,
            description: fromSignaturePage.description,
          }),
          marketingBreadcrumbJsonLd(fromSignaturePage.title, fromSignaturePage.path),
        ]}
      />
      <div
        aria-hidden
      ...
      <section className="container pb-16 sm:pb-20">
        <RevealOnScroll className="mx-auto max-w-2xl text-center" delayMs={40}>
          ...
          <p className="mt-3 text-sm text-muted-foreground sm:text-base">
            Signatures, promotional blocks, analytics, and guides for better outbound email.
          </p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Button asChild variant="outline">
              <Link href="/signatures">Signatures</Link>
            </Button>
            <Button asChild variant="outline">
