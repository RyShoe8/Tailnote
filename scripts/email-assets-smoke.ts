...
async function main() {
  ...
  assert.ok(icons.length >= 7, 'expected icon-*.png files');
  ...
  const html =
    '<img src="https://app.example.com/email-assets/icon-linkedin.png?v=11" width="18" height="18">';
  const noop = await inlineEmailAssetIcons(html);
  assert.equal(noop, html, 'inline: no-op without browser fetch');

  assert.equal(
    resolveEmailAssetIconUrl('/email-assets/icon-linkedin.png?v=11', 'https://app.example.com'),
    'https://app.example.com/email-assets/icon-linkedin.png?v=11'
  );
  assert.equal(
    resolveEmailAssetIconUrl(
